...
const validateCred = (arr) => {
  ...
  sum += checkDigit; // Añadimos el dígito de verificación al total
  
  // Iteramos desde el penúltimo dígito hacia la izquierda
  for (let i = arr.length - 2; i >= 0; i--) {
    let value = arr[i];
    // Duplicamos cada segundo dígito (posiciones impares desde la derecha)
    if ((arr.length - 1 - i) % 2 === 1) {
      value *= 2;
      // Si es mayor que 9, restamos 9
      if (value > 9) {
        value -= 9;
      }
    }
  ...
  // Verificamos si el módulo 10 es 0
  return sum % 10 === 0;
}

// console.log(validateCred(valid1));

// Step 4, Return an array with the invalid cards
...
// Step 5, Return an array with the companies who sent those invalid cards
const idInvalidCardsCompanies = nestedArr => {
  let companiesWithInvalidCards = [];
  nestedArr.forEach((arr) => {
    switch(arr[0]){
      case 3:
        if(companiesWithInvalidCards.includes('Amex')){
          break;
        } else {
          companiesWithInvalidCards.push('Amex');
          break;
        }
      case 4:
...
console.log(idInvalidCardsCompanies(findInvalidCards(batch)));
